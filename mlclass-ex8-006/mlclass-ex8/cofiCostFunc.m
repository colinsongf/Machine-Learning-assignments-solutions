...
M=(t-Y).^2;
t1=sum(sum(R.*M))/2;
t2=sum(sum(Theta.^2))*lambda/2;
t3=sum(sum(X.^2))*lambda/2;
J=t1+t2+t3;
...
%    size(X(i,:))
 %   size(Theta')
  %  size(Ytemp)
   % Theta_grad(i,:) = (Xtemp(i,:)*Thetatemp'-Ytemp)*Xtemp;

%end
    
 %for j=1:size(Theta_grad,1)
     %for k=1:size(Theta_grad,2)
     %for i=1:size(X,1)
     %Theta_grad(j,k)=Theta_grad(j,k)+(((Theta(j,:)*X(i,:)')-Y(i,j)).*R(i,j))*X(i,k);
     %end
     %end
   %  temp=(X*Theta(j,:)').*R(:,j);
  %   Theta_grad(j,:)=(temp'*X);
